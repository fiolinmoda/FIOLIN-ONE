...
export function PurchaseInvoicePage() {
  ...
  function updatePurchaseOrder(value: string | null) {
    ...
    setInvoiceInput((current) => {
      const quantity = firstItem ? Math.max(firstItem.receivedQuantity, 0) || 1 : current.items[0].quantity
      const unitPrice = firstItem?.unitPrice ?? current.items[0].unitPrice
      ...
      return {
        ...
        invoiceAmount: firstItem ? totalAmount : current.invoiceAmount,
        items: firstItem
          ? [
              {
          ...
          <DialogContent sx={dialogContentSx}>
            ...
            <Stack spacing={2.5} sx={{ pt: 1 }}>
              ...
              <Stack direction={{ xs: 'column', md: 'row' }} spacing={2}>
                ...
                <TextField select label="Satın Alma Siparişi" value={invoiceInput.purchaseOrderId ?? ''} onChange={(event) => updatePurchaseOrder(event.target.value || null)} fullWidth>
