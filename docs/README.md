# FIOLIN ONE Documentation

## Purpose

This documentation hub organizes architecture, business, database, API, UI, workflow, deployment, and decision records for FIOLIN ONE.

## Status

Draft

## Last Updated

2026-07-05

## Documentation Map

- [System Architecture Blueprint](architecture/system-architecture-blueprint.md)
- [Architecture](architecture/system-overview.md)
- [Business](business/product-lifecycle.md)
- [Fabric Management Business Analysis](business/fabric-management.md)
- [Purchasing Management Business Analysis](business/purchasing-management.md)
- [Product Development Business Analysis](business/product-development.md)
- [Database Blueprint](database/database-blueprint.md)
- [Database](database/er-diagram.md)
- [API Blueprint](api/api-blueprint.md)
- [API](api/api-standards.md)
- [UI](ui/ui-standards.md)
- [Workflows](workflows/barcode-workflow.md)
- [Deployment](deployment/docker.md)
- [Decisions](decisions/adr-001-product-centric-architecture.md)

## Future Content

- Ownership model
- Review cadence
- Documentation standards
- Release documentation process
